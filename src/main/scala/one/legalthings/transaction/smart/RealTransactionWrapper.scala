...
import one.legalthings.lang.v1.traits.{Proven, _}
import one.legalthings.state._
import scodec.bits.ByteVector
import one.legalthings.account.{Address, AddressOrAlias, Alias}
import one.legalthings.transaction._
import one.legalthings.transaction.assets._
import one.legalthings.transaction.assets.exchange.OrderType.{BUY, SELL}
import one.legalthings.transaction.assets.exchange.{AssetPair, ExchangeTransaction, Order}
    ...
    tx match {
      ...
      case ms: MassTransferTransaction =>
        Tx.MassTransfer(
          ...
          transfers = ms.transfers.map(r => one.legalthings.lang.v1.traits.TransferItem(r.address, r.amount)).toIndexedSeq,
