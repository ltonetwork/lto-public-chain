...
import one.legalthings.mining.{Miner, MinerDebugInfo}
import one.legalthings.network.{LocalScoreChanged, PeerDatabase, PeerInfo, _}
import one.legalthings.settings.WavesSettings
import one.legalthings.state.{ByteStr, LeaseBalance, NG, Portfolio}
import one.legalthings.transaction._
import one.legalthings.utils.{Base58, ScorexLogging}
  ...
  @ApiImplicitParams(
    Array(
      new ApiImplicitParam(
        ...
        required = true,
        paramType = "body",
        dataType = "one.legalthings.http.DebugMessage",
        defaultValue = "{\n\t\"message\": \"foo\"\n}"
      )
    ))
  ...
  @ApiImplicitParams(
    Array(
      new ApiImplicitParam(
        ...
        required = true,
        paramType = "body",
        dataType = "one.legalthings.http.RollbackParams",
