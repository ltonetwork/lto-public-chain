...
import one.legalthings.account.{Address, PublicKeyAccount}
import one.legalthings.api.http.DataRequest._
import one.legalthings.api.http.alias.{CreateAliasV1Request, CreateAliasV2Request, SignedCreateAliasV1Request, SignedCreateAliasV2Request}
import one.legalthings.api.http.assets.SponsorFeeRequest._
import one.legalthings.api.http.assets._
import one.legalthings.api.http.leasing._
import one.legalthings.http.BroadcastRoute
import one.legalthings.settings.{FunctionalitySettings, RestAPISettings}
import one.legalthings.state.diffs.CommonValidation
  ...
  private def txToExtendedJson(tx: Transaction): JsObject = {
    ...
    tx match {
      case lease: LeaseTransaction =>
        import one.legalthings.transaction.lease.LeaseTransaction.Status._
