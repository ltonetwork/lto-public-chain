...
import one.legalthings.crypto.SignatureLength
import one.legalthings.db.WithState
import one.legalthings.state.reader.LeaseDetails
import one.legalthings.{TestTime, TransactionGen}
...
import one.legalthings.transaction.ValidationError.AliasDoesNotExist
import one.legalthings.transaction.assets.{IssueTransactionV1, ReissueTransactionV1}
import one.legalthings.transaction.lease.{LeaseCancelTransactionV1, LeaseTransactionV1}
import one.legalthings.transaction.smart.SetScriptTransaction
import one.legalthings.transaction.transfer._
import one.legalthings.transaction.{CreateAliasTransactionV1, DataTransaction, GenesisTransaction}
import one.legalthings.features._
import one.legalthings.settings.{FunctionalitySettings, TestFunctionalitySettings}
import one.legalthings.{NoShrink, TestTime, TransactionGen, history}
    ...
    "forget rollbacked transaction for quering" in forAll(accountGen, accountGen, Gen.nonEmptyListOf(Gen.choose(1, 10))) {
      case (sender, recipient, txCount) =>
        ...
        withDomain(wavesSettings) { d =>
          d.appendBlock(genesisBlock(nextTs, sender, one.legalthings.state.diffs.ENOUGH_AMT))
