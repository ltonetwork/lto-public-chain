...
import one.legalthings.account.PublicKeyAccount._
import one.legalthings.account._
import one.legalthings.lang.Global
import one.legalthings.settings.Constants
import one.legalthings.state._
import one.legalthings.state.diffs.ENOUGH_AMT
import one.legalthings.transaction._
import one.legalthings.transaction.assets._
import one.legalthings.transaction.assets.exchange._
...
import one.legalthings.transaction.transfer.MassTransferTransaction.{MaxTransferCount, ParsedTransfer}
import one.legalthings.transaction.transfer._
import one.legalthings.utils.TimeImpl
...
trait TransactionGenBase extends ScriptGen {
  ...
  def createWavesTransfer(sender: PrivateKeyAccount,
    ...
    TransferTransactionV1.selfSigned(sender, recipient, amount, timestamp, fee, Array())
