package one.legalthings.network.peer
...
import one.legalthings.network.PeerDatabaseImpl
import one.legalthings.settings.NetworkSettings
...
class PeerDatabaseImplSpecification extends path.FreeSpecLike with Matchers {
  ...
  private val settings1 = config1.as[NetworkSettings]("lto.network")

  private val config2   = ConfigFactory.parseString("""lto.network {
      |  file = null
      |  known-peers = []
      |  peers-data-residence-time: 10s
      |}""".stripMargin).withFallback(ConfigFactory.load()).resolve()
  private val settings2 = config2.as[NetworkSettings]("lto.network")
    ...
    "if blacklisting is disabled" - {
      "should clear blacklist at start" in {
        ...
        val prevConfig   = ConfigFactory.parseString(s"""lto.network {
             ...
             |}""".stripMargin).withFallback(ConfigFactory.load()).resolve()
        val prevSettings = prevConfig.as[NetworkSettings]("lto.network")
        val prevDatabase = new PeerDatabaseImpl(prevSettings)
        prevDatabase.blacklist(address1, "I don't like it")
        prevDatabase.close()

        val config   = ConfigFactory.parseString(s"""lto.network {
             |  file = "$path"
             |  known-peers = []
             |  peers-data-residence-time: 100s
             |  enable-blacklisting = no
             |}""".stripMargin).withFallback(ConfigFactory.load()).resolve()
        val settings = config.as[NetworkSettings]("lto.network")
