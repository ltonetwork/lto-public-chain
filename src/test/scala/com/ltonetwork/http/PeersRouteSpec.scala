...
class PeersRouteSpec extends RouteSpec("/peers") with RestAPISettingsHelper with ScalaCheckDrivenPropertyChecks with MockFactory {
  ...
  routePath("/connect") in {
    ...
    Post(connectUri, "") ~> ApiKey(apiKey) ~> route ~> check(handled shouldEqual false)
    Post(connectUri, Json.obj()) ~> ApiKey(apiKey) ~> route ~> check {
    ...
    val result = Post(connectUri, ConnectReq(address.getHostName, address.getPort)) ~> ApiKey(apiKey) ~> route ~> runRoute
    check {
