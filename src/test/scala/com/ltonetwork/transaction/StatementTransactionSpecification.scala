...
import com.ltonetwork.api.requests.StatementRequest
...
class StatementTransactionSpecification extends AnyPropSpec with ScalaCheckDrivenPropertyChecks with Matchers with TransactionGen {

  private def checkSerialization(tx: StatementTransaction): Assertion = {
    val tryParse = StatementTransaction.parseBytes(tx.bytes())
    ...
    parsed.statementType shouldEqual tx.statementType
  ...
  property("serialization roundtrip") {
    forAll(statementTransactionGen)(checkSerialization)
  }

  property("from TransactionBuilder") {
    forAll(statementTransactionGen) { tx: StatementTransaction =>
  ...
  property("JSON roundtrip") {
    forAll(statementTransactionGen) { tx =>
      val json = tx.json()
      json.toString shouldEqual tx.toString

      val req = json.as[StatementRequest]
  ...
  property(testName = "JSON format validation (minimal)") {
    val js = Json.parse("""{
                       "type": 23,
                       "version": 3,
                       "id": "3CZx7qk6KUGpwEc4siVRhbJK1WbL6SnUuysycSxTNzBx",
  ...
  property(testName = "JSON format validation (full)") {
    val js = Json.parse("""{
                       "type": 23,
                       "version": 3,
                       "id": "4B1nZSGyYsL7DZ79gGpcRTkhJcfFLovjC9xwLnmkhqr9",
                       "sender": "3Mr31XDsqdktAdNQCdSd8ieQuYoJfsnLVFg",
                       "senderKeyType": "ed25519",
                       "senderPublicKey": "FM5ojNqW7e9cZ9zhPYGkpSP1Pcd8Z3e3MNKYVS5pGJ8Z",
