...
def generate_account(key_type='ed25519', private_key=None):
    factory = AccountFactory(CHAIN_ID) if key_type == 'ed25519' else AccountFactoryECDSA(CHAIN_ID, key_type)

    if private_key:
        return factory.create_from_private_key(private_key)
    else:
