...
import com.emstlk.nacl4s.crypto.{VerifyKey, SigningKey, SigningKeyPair}
import org.openjdk.jmh.annotations._

// Generate a key pair which allow you sign any message and anybody can verify it with your public key.

@State(Scope.Thread)
class NaCl_PK_SignatureVerify_Benchmark {
  ...
  def setUp(): Unit = {
    ...
    signature = SigningKey(keys.privateKey).sign(message)
  }
