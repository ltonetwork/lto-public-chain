...
trait BaseState extends TransactionGenBase {
  ...
  private val initGen: Gen[(PrivateKeyAccount, Block)] = for {
    ...
    val genesisTx = GenesisTransaction.create(rich, lto(100000000L), System.currentTimeMillis() - 10000).explicitGet()
  ...
  private def append(prev: Option[Block], next: Block): Unit = {
    ...
    state.append(preconditionDiff, 0, next) // fixing compilation
  }
