...
object Global extends BaseGlobal {
  ...
  def signatureVerify(message: Array[Byte], sig: Array[Byte], pub: Array[Byte]): Boolean =
    Try(new Ed25519().verify(message, sig, pub)).fold(_ => false, _ => true)

  def keccak256(message: Array[Byte]): Array[Byte]  = new Hasher("Keccak-256").hash(message).getBytes
  def blake2b256(message: Array[Byte]): Array[Byte] = new Hasher("Blake2b-256").hash(message).getBytes
  def sha256(message: Array[Byte]): Array[Byte]     = new Hasher("SHA-256").hash(message).getBytes
}
