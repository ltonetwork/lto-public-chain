...
import one.legalthings.api.http.AddressApiRoute
import one.legalthings.api.http.assets.SignedIssueV1Request
import one.legalthings.http.DebugMessage
...
object SyncHttpApi extends Assertions {
  ...
  implicit class NodeExtSync(n: Node) extends Assertions with Matchers {
    ...
    import one.legalthings.it.api.AsyncHttpApi.{NodeAsyncHttpApi => async}
  ...
  implicit class NodesExtSync(nodes: Seq[Node]) {
    ...
    import one.legalthings.it.api.AsyncHttpApi.{NodesAsyncHttpApi => async}
    ...
    def rollback(height: Int, returnToUTX: Boolean = true): Unit = {
      Await.result(
        Future.traverse(nodes) { node =>
          one.legalthings.it.api.AsyncHttpApi.NodeAsyncHttpApi(node).rollback(height, returnToUTX)
        },
        ConditionAwaitTime
      )
    }
    ...
    def waitForHeight(height: Int): Unit = {
      Await.result(
        Future.traverse(nodes) { node =>
          one.legalthings.it.api.AsyncHttpApi.NodeAsyncHttpApi(node).waitForHeight(height)
        },
        ConditionAwaitTime
      )
    }
